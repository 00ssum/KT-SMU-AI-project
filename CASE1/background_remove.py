...
# for i in range(1,81):
file_path = './output/1.detection/'
file_list = os.listdir('./output/1.detection')
for file in file_list :
    src = cv2.imread(file_path+file)
    ...
    while True:
        ...
        if key == 13:  # ENTER
            ...
            tmp = cv2.cvtColor(dst, cv2.COLOR_RGB2GRAY)
            _, alpha = cv2.threshold(tmp, 0, 255, cv2.THRESH_BINARY)
            r, g, b = cv2.split(dst)
            rgba = [r, g, b, alpha]
            dst = cv2.merge(rgba, 4)
            cv2.imwrite('./change/'+file, dst)
            break
